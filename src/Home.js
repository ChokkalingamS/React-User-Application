...
export function Home() {
    ...
    axios(
      {
        url: `${url}/getusers`,
        method: 'GET'
      }).then(response => response.data).then(data => {setRows(data);setProgress(0)}).catch();
  };
  ...
  return <div className='home'>
    ...
    <div>
      {(rows.length) ? <TableView rows={rows} removeUser={removeUser}/>: (progress === 1) && <CircularProgress id='addprogress' color='success'></CircularProgress>}
    </div>

    <div>
    {(rows.length) ? <Tab rows={rows} removeUser={removeUser}/>: (progress === 1) && <CircularProgress id='tab-progress' color='success'></CircularProgress>}
    </div>
...
{
      return <TableContainer component={Paper} id="table">
...
function Features({ id, removeUser,show }) {
  ...
  return <div className='features'>
    <Tooltip title="Edit">
    {(!show)?<IconButton onClick={() => history.push(`/edituser/${id}`)}><ModeEditIcon color="success" /></IconButton>:
    <Button variant='contained' color='warning' onClick={() => history.push(`/edituser/${id}`)}>Edit</Button>}
    </Tooltip>
